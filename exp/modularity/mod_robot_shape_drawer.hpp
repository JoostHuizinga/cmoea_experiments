...
*/
#ifndef MOD_ROBOT_SHAPE_DRAWER_H
#define MOD_ROBOT_SHAPE_DRAWER_H
...
#include <windows.h>
#endif
#include "modules/bullet/GLDebugFont.h"
#include "modules/bullet/GlutStuff.h"
...
public:
	ModRobotShapeDrawer(){
		...
		virtual void drawOpenGL(btScalar* m, const btCollisionShape* shape, const btVector3& color,int	debugMode,const btVector3& worldBoundsMin,const btVector3& worldBoundsMax, int special = 0){

		    if (shape->getShapeType() == CUSTOM_CONVEX_SHAPE_TYPE)
		    {
		        std::cout << "Drawing a custom convex shape." << std::endl;
		...
		//      dz *= halfExtent[2];
		    ...
		    {
		        std::cout << "Drawing a box shape proxytype lines." << std::endl;
		        btVector3 org(m[12], m[13], m[14]);
		        btVector3 dx(m[0], m[1], m[2]);
		        btVector3 dy(m[4], m[5], m[6]);
		        btVector3 dz(m[8], m[9], m[10]);
		    ...
		    {
		        std::cout << "Drawing a uniform scaling shape proxytype." << std::endl;
		    ...
		    {
		        std::cout << "Drawing a compound shape proxytype." << std::endl;
		    ...
		    {
		        //std::cout << "Drawing something with texture?" << std::endl;
		        if(m_textureenabled&&(!m_textureinitialized))
		        {
		            ...
		            glTexParameterf(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR_MIPMAP_LINEAR);  //JH: According to OpenGL documentation, this option is invalid.
		        ...
		        if (!(debugMode & btIDebugDraw::DBG_DrawWireframe))
		        {
		            ...
		            switch (shapetype)
		            {
		                ...
		                {
		                    std::cout << "Drawing a sphere shape proxytype." << std::endl;
		                ...
		                {
		                    //std::cout << "Drawing a box shape proxytype." << std::endl;
		                    const btBoxShape* boxShape = static_cast<const btBoxShape*>(shape);
		                    btVector3 halfExtent = boxShape->getHalfExtentsWithMargin();
		                    ...
		                    for (int i=0;i<si;i+=3)
		                    {
		                        ...
		                        normal.normalize ();
		                        //JH: strong suspicion original normals were wrong
...
//		                        glNormal3f(normal.getX(),normal.getY(),normal.getZ());
		            ...
		            case CONE_SHAPE_PROXYTYPE:
		                {
		                    std::cout << "Drawing a cone shape proxytype.\n" << std::endl;
		            ...
		            case STATIC_PLANE_PROXYTYPE:
		                {
		                    std::cout << "Drawing a static plane proxytype.\n" << std::endl;
		...
		*/
		            ...
		            {
		                std::cout << "Drawing a multi sphere shape proxytype." << std::endl;
		            ...
		            default:
		                {
		                    ...
		                    if (shape->isConvex())
		                    {
		                        ...
		                        if (poly)
		                        {
		                            std::cerr << "Drawing convex polyhedron is not currently supported." << std::endl;
//		                            //std::cout << "...Shape is polyhedral." << std::endl;
//		                            int i;
//		                            glBegin (GL_TRIANGLES);
...
//		                            glEnd ();
		                        ...
		                        {
		                            //std::cout << "...Shape is not polyhedral." << std::endl;
                                    //JH: Another giant hack, just assume the center of the object is at 0,0,0
                                    btVector3 center(0,0,0);
		                            ...
		                            if (hull->numTriangles () > 0)
		                            {
		                                ...
		                                for (int i = 0; i < hull->numTriangles (); i++)
		                                {
		                                    ...
		                                    if(special == 1){
		                                        if(normal.getX() < -0.5 && normal.getY()<0){
		                                            glColor3f(1.f,0.7f, 0.2f);
		                                        } else if (normal.getZ() > -0.3 && normal.getZ() < 0.3 && normal.getY()<0) {
		                                    ...
		                                    //JH: I believe most of their normals are pointing the wrong way
		                                    //(which they solved by having a light on both the top an the bottom)
//		                                    glNormal3f(-normal.getX(),-normal.getY(),-normal.getZ());
//		                                    //JH: This is the original method
//                                            //glNormal3f(normal.getX(),normal.getY(),normal.getZ());
//                                            glVertex3f (v1.x(), v1.y(), v1.z());
//                                            glVertex3f (v2.x(), v2.y(), v2.z());
//                                            glVertex3f (v3.x(), v3.y(), v3.z());

		                                    //JH: This is part of the hack
                                            btVector3 n1 = v1;
                                            btVector3 n2 = v2;
                                            btVector3 n3 = v3;
		                                    n1.normalize();
...
#endif //MOD_ROBOT_SHAPE_DRAWER_H
