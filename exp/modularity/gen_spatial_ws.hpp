...
#ifndef EXP_MODULARITY_GEN_SPATIAL_WS_HPP_
#define EXP_MODULARITY_GEN_SPATIAL_WS_HPP_
...
#include <modules/modularity2/opt_placement.hpp>
#include <modules/modularity2/modularity.hpp>
...
class DnnSpatialWS : public Dnn<N, C, Params>
{
public:
    typedef N neuron_t;
    typedef C conn_t;
    typedef typename gen::DnnSpatialWS<neuron_t, conn_t, Params> nn_t;
    ...
    typedef mod::null_model::Layered<graph_t> l_null_model_t;

    void init(){
        dbg::trace trace1("gen_spatial", DBG_HERE);
        dbg::trace trace2("develop", DBG_HERE);
        Dnn<N, C, Params>::init();
    }

    size_t nb_layers() const {return Params::dnn::spatial::layers_size(); }
    size_t layer_size(size_t n) const {dbg::trace trace("gen_spatial", DBG_HERE); return Params::dnn::spatial::layers(n); }
    size_t get_depth() const {return nb_layers() + 1; }
    const nn_t get_nn() const{dbg::trace trace("gen_spatial", DBG_HERE); return *this;}
    nn_t get_nn(){dbg::trace trace("gen_spatial", DBG_HERE); return *this;}
    ...
    void random(){
        ...
        assert(_layers[0].size() == Params::dnn::nb_inputs);
        dbg::out(dbg::info, "rand_indv") << "Adding neurons" << std::endl;
        for (size_t i = 1; i < nb_layers() - 1; ++i){
            std::vector<vertex_desc_t> layer;
            for (size_t j = 0; j < Params::dnn::spatial::layers(i); ++j){
                vertex_desc_t v = this->add_neuron("n" + boost::lexical_cast<std::string>(i));
        ...
        dbg::out(dbg::info, "rand_indv") << "Adding connections" << std::endl;
        size_t nb_conns = misc::rand(Params::dnn::min_nb_conns, Params::dnn::max_nb_conns);
        for (size_t i = 0; i < nb_conns; ++i){
            this->_add_conn();
        }
        _rand_obj = misc::rand<float>();
    ...
    float rand_obj() const {dbg::trace trace("gen_spatial", DBG_HERE); return _rand_obj; }

    void cross(const DnnSpatialWS& o, DnnSpatialWS& c1, DnnSpatialWS& c2) {
        dbg::trace trace("gen_spatial", DBG_HERE);
        if (misc::flip_coin())
        {
            c1 = *this;
    ...
    std::map<vertex_desc_t, spatial::Pos> get_optimal_coordsmap(){
        dbg::trace trace1("gen_spatial", DBG_HERE);
        dbg::trace trace2("optlength", DBG_HERE);
        assert(_layers.size());
...
//            mod::split(nn.get_graph(), mods, null_model);
//            std::vector<SvgColor> colors = getColors(nn.get_graph(), mods);
//
//            dbg::out(dbg::info, "optlength") << "Writing graph..." << std::endl;
//            std::ofstream ofs(svg_name.c_str());
//            SvgWriter writer;
//            float scale = Params::visualisation::scale;
...
//        }
    ...
    std::vector<SvgColor> getColors(const std::vector<std::string>& mods){
        return getColors(this->get_graph(), mods);
    }


    std::vector<SvgColor> getColors(graph_t& graph, const std::vector<std::string>& mods){
        dbg::trace trace("gen_spatial", DBG_HERE);
    ...
    float compute_length(){
        ...
        BGL_FORALL_EDGES_T(e, this->_g, graph_t)
        {
            ...
            dbg::out(dbg::info, "length") << "Source pos: " << _coords_map[src] <<
                    " target pos: " << _coords_map[tgt] <<
                    " distance: " << d << std::endl;
            length += d * d;
        }
        return length;
    ...
    const layer_map_t& getLayerMap() const{
        return _layer_map;
    }

    l_null_model_t getLayeredNullModel() const{
        dbg::trace trace("gen_spatial", DBG_HERE);
        std::map<vertex_desc_t, int> layers;
        BGL_FORALL_VERTICES_T(v, this->get_graph(), graph_t){
            dbg::out(dbg::info, "null_model") << "Vertex: " << v << " mapped to: " <<  _layer_map.find(v)->second.first << std::endl;
            layers[v] = _layers.size() - 1 - _layer_map.find(v)->second.first;
        }
        return l_null_model_t(this->get_graph(), layers);
    }
    ...
    void _reinit_layers()
    {
        ...
        for (size_t i = 0, k = 0; i < _layers.size(); ++i){
            for (size_t j = 0; j < _layers[i].size(); ++j, ++k)
            {
                ...
                dbg::out(dbg::info, "length") << "Neuron: " << k << " has position: " << _coords[k] << std::endl;
            }
        }
    }
    ...
    void _copy_coords()
    {
        ...
        for (size_t i = 0, k = 0; i < Params::dnn::spatial::layers_size(); ++i)
            for (size_t j = 0; j < Params::dnn::spatial::layers(i); ++j, ++k)
            {
                ...
                dbg::out(dbg::info, "length") << "Index: " << k <<
                        " x: " << Params::dnn::spatial::x(k) <<
                        " y: " << Params::dnn::spatial::y(k) <<
                        " z: " << Params::dnn::spatial::z(k) << std::endl;
...
//        size_t max_tries = num_vertices(this->_g) * num_vertices(this->_g), nb_tries = 0;
//        do{
...
//    }
    ...
    void _add_conn(){
        typedef std::pair<vertex_desc_t, vertex_desc_t> vertex_pair_t;
        std::vector<vertex_pair_t> candidates;
        vertex_desc_t src, tgt;
        for(size_t layer_i=0; layer_i<_layers.size()-1; ++layer_i){
            for(size_t source_i=0; source_i<_layers[layer_i].size(); ++source_i){
                src = _layers[layer_i][source_i];
                for(size_t target_i=0; target_i<_layers[layer_i+1].size(); ++target_i){
                    tgt = _layers[layer_i+1][target_i];
                    if(!is_adjacent(this->_g, src, tgt)){
                        candidates.push_back(vertex_pair_t(src, tgt));
                    }
                }
            }
        }
        ...
        if(!candidates.empty()){
            vertex_pair_t vertex_pair = misc::rand_in_vector(candidates);
            typename nn_t::weight_t w; // = nn::trait<typename
            w.random();
            this->add_connection(vertex_pair.first, vertex_pair.second, w);
        }
    }
...
#endif /* EXP_MODULARITY_GEN_SPATIAL_WS_HPP_ */
