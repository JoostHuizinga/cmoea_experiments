...
namespace OGLFT {
  ...
  Face::Face ( const char* filename, float point_size, FT_UInt resolution , FT_Long style_flags)
    : point_size_( point_size ), resolution_( resolution )
  {
    ...
    //std::cout << ft_face->num_faces  << std::endl;
    for (size_t i=0; i<ft_face->num_faces; ++i){
            FT_Error error = FT_New_Face( Library::instance(), filename, i, &ft_face );
            if ( error != 0 ) {
                valid_ = false;
                continue;
            }
            if(ft_face->style_flags == style_flags){
                break;
            }
            //std::cout << "Face: " << i << " family: " << ft_face->family_name << " style: " << ft_face->style_name << " flags: " << ft_face->style_flags << std::endl;
    }
  ...
  Raster::Raster ( const char* filename, float point_size, FT_UInt resolution, FT_Long style_flags )
    : Face( filename, point_size, resolution, style_flags )
  {
    if ( !isValid() ) return;
  ...
  Monochrome::Monochrome ( const char* filename, float point_size, FT_UInt resolution, FT_Long style_flags )
    : Raster( filename, point_size, resolution, style_flags )
  {}
  ...
  void Monochrome::renderGlyph ( FT_Face face, FT_UInt glyph_index )
  {
    ...
    glBitmap( bitmap_glyph->bitmap.width, bitmap_glyph->bitmap.rows,
	      -bitmap_glyph->left,
	      ((int)bitmap_glyph->bitmap.rows) - bitmap_glyph->top,
  ...
  Grayscale::Grayscale ( const char* filename, float point_size,
			   FT_UInt resolution, FT_Long style_flags )
    : Raster( filename, point_size, resolution, style_flags )
  {}
...
//              *inverse_ptr++ = 0xff;
  ...
  {
//      std::cerr << "Rendering grayscale glyph" << std::endl;
    FT_Error error = FT_Load_Glyph( face, glyph_index, FT_LOAD_DEFAULT );

    if ( error != 0 ){
        std::cerr << "WARNING: Error on loading glyph" << std::endl;
        return;
    }
    ...
    if ( error != 0 ){
        std::cerr << "WARNING: Error on retrieving glyph" << std::endl;
        return;
    }
    ...
    if ( error != 0 ){
        std::cerr << "WARNING: Error on copying glyph" << std::endl;
        return;
    }
    ...
    if ( error != 0 ) {
        std::cerr << "WARNING: Error on transforming glyph" << std::endl;
      FT_Done_Glyph( glyph );
      return;
    }
    ...
    //std::cout << "Setting bitmap position: " << bitmap_glyph->left << " " << bitmap_glyph->top << " " <<  bitmap_glyph->bitmap.rows<< std::endl;
    glBitmap( 0, 0, 0, 0,
	      bitmap_glyph->left,
	      bitmap_glyph->top - ((int)bitmap_glyph->bitmap.rows),
	      0 );
...
//    std::cout << "Drawing pixels somewhere:" << bitmap_glyph->bitmap.width << ", " << bitmap_glyph->bitmap.rows << std::endl;
...
//    std::cout << "Advancing pixel buffer" << std::endl;
    glBitmap( 0, 0, 0, 0,
  ...
  Polygonal::Polygonal ( const char* filename, float point_size, FT_UInt resolution , FT_Long style_flags )
    : Face( filename, point_size, resolution, style_flags )
  {
    if ( !isValid() ) return;
  ...
  Outline::Outline ( const char* filename, float point_size, FT_UInt resolution , FT_Long style_flags )
    : Polygonal( filename, point_size, resolution, style_flags )
  {
    if ( !isValid() ) return;
  ...
  Filled::Filled ( const char* filename, float point_size, FT_UInt resolution , FT_Long style_flags )
    : Polygonal( filename, point_size, resolution , style_flags )
  {
    if ( !isValid() ) return;
  ...
  Solid::Solid ( const char* filename, float point_size, FT_UInt resolution, FT_Long style_flags  )
    : Filled( filename, point_size, resolution, style_flags  )
  {
    if ( !isValid() ) return;
  ...
  Texture::Texture ( const char* filename, float point_size, FT_UInt resolution, FT_Long style_flags, int filter_size )
    : Face( filename, point_size, resolution, style_flags  )
  {
  ...
  MonochromeTexture::MonochromeTexture ( const char* filename, float point_size,
					 FT_UInt resolution, FT_Long style_flags )
    : Texture( filename, point_size, resolution, style_flags )
  {}
  ...
  TranslucentTexture::TranslucentTexture ( const char* filename, float point_size,
					   FT_UInt resolution, FT_Long style_flags, int filter_size)
    : Texture( filename, point_size, resolution, style_flags, filter_size )
  {}
  ...
  GLubyte* TranslucentTexture::createBlurredTexture (GLubyte* source, int* width, int* height, int bitwidth, int bitheight )
  {
      int new_width = nearestPowerCeil(bitwidth+2*_filter_size);
      int new_height = nearestPowerCeil(bitheight+2*_filter_size);
      GLubyte* result = new GLubyte[ 2 * new_width * new_height ];
      for ( int r = 0; r < new_height; r++ ) {
          for ( int p = 0; p < new_width; p++ ) {
              int devider = 0;
              int newValue = 0;
              for ( int xd = -_filter_size; xd <= _filter_size; xd++ ) {
                  for ( int yd = -_filter_size; yd <= _filter_size; yd++ ) {
                      int x = p + xd - _filter_size;
                      int y = r + yd - _filter_size;
                      if(x > 0 && x < (bitwidth) && y > 0 && y < (bitheight)){
                          newValue += source[2*((*width)*y+x)+1];
                      }
                      devider += 1;
                  }
              }
              newValue /= devider;
              result[2*(new_width*r+p)] = 0xff;
              result[2*(new_width*r+p)+1] = newValue;
          }
      }
      *width=new_width;
      *height=new_height;
  ...
  void TranslucentTexture::bindTexture ( FT_Face face, FT_UInt glyph_index )
  {
    ...
    // TEST START //
    //int width, height;
    //FT_Bitmap* test = blurPixmap(face->glyph->bitmap);
    //GLubyte* inverted_pixmap = invertPixmap(*test, &width, &height );
    //delete test;
    // TEST FAILED //
    //
    // Texture maps have be a power of 2 in size (is 1 a power of 2?), so
    ...
    if(_filter_size>0){
        GLubyte* temp_pixmap = createBlurredTexture( inverted_pixmap, &width, &height, face->glyph->bitmap.width, face->glyph->bitmap.rows);
        delete[] inverted_pixmap;
        inverted_pixmap = temp_pixmap;
    }
    ...
    // Save a good bit of the data about this glyph
    texture_info.left_bearing_ = face->glyph->bitmap_left - _filter_size;
    texture_info.bottom_bearing_ = -( face->glyph->bitmap.rows - face->glyph->bitmap_top + _filter_size);
    texture_info.width_ = face->glyph->bitmap.width+2*_filter_size;
    texture_info.height_ = face->glyph->bitmap.rows+2*_filter_size;
    texture_info.texture_s_ = (GLfloat)texture_info.width_ / width;
    texture_info.texture_t_ = (GLfloat)texture_info.height_ / height;
    texture_info.advance_ = face->glyph->advance;
